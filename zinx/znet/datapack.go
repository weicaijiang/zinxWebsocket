...
//封包
func (dp *DataPack) Pack(imsg ziface.IMessage) ([]byte, error) {
	...
	if err != nil {
		...
		return nil, errors.New("DataPack Pack Marshal err")
	}
	//第二步base64加密
	...
	if len(jsonData) > int(utils.GlobalObject.MaxPackageSize) {
		...
		return nil, errors.New("DataPack Pack msg len big then MaxPackageSize err")
	}
...
//拆包
func (dp *DataPack) Unpack(messageType int, data []byte) (ziface.IMessage, error) {
